...
class PostsModel(db.Model):
    ...
    lastCommentedAt: Mapped[DATETIME] = mapped_column(DATETIME, nullable=False)
    lastCommentedUserId: Mapped[VARCHAR] = mapped_column(VARCHAR(255), ForeignKey('user.id'), nullable=False)
    commentsCount: Mapped[INT] = mapped_column(INT, nullable=False)
    likesCount: Mapped[INT] = mapped_column(INT, nullable=False)
    content: Mapped[VARCHAR] = mapped_column(VARCHAR(255), nullable=False)

    # 定义author成员变量与UsersModel映射类的关系
    author = relationship("UsersModel", back_populates="posts")
