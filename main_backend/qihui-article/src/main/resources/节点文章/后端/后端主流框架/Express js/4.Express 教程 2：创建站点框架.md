...
+   [上一页](/zh-CN/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs/Tutorial_local_library_website)
+   [概述：Express Web 框架（Node.js/JavaScript）](/zh-CN/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs)
+   [下一页](/zh-CN/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs/mongoose)

本节将演示如何创建一个可添加路由、模板/视图、和数据库调用的“骨架”站点。

<table><tbody><tr><th scope="row">前提：</th><td><a href="/zh-CN/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs/development_environment">配置 Node 开发环境</a>。复习 Express 教程。</td></tr><tr><th scope="row">目标：</th><td>掌握用 <strong>Express 应用生成器</strong> 创建站点的方法。</td></tr></tbody></table>
...
![express 生成器的帮助信息](/zh-CN/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs/skeleton_website/express-help.png)
...
![用 express 生成器生成一个应用](/zh-CN/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs/skeleton_website/express.png)
...
3.  最后在浏览器中导航至 [http://localhost:3000/](http://localhost:3000/) ，就可以访问该应用。你应该可以看到： ![Express 应用生成器生成的应用启动成功](/zh-CN/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs/skeleton_website/success.png)
    

一个 Express 应用就配置成功了，它托管于 localhost:3000。

**备注：** 指定 DEBUG 变量可启用控制台日志记录/调试。例如，当你访问上面的页面时，你会看到像这样的调试输出： ![用 npm start 启动这个应用](/zh-CN/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs/skeleton_website/npm-start.png) 直接通过 `npm start` 命令启动应用也可以，但不会看到调试信息。
...
**备注：** 用 node 启动该应用并访问 [http://localhost:3000/users/](http://localhost:3000/users/)，浏览器会返回一条消息：'respond with a resource'。
