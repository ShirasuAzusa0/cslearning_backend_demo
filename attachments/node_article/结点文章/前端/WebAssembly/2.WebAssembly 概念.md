...
创建 WASM 代码的众多选项中有两个是在线 WASM 汇编程序或 Emscripten。有许多在线 WASM 汇编程序可供选择，例如：
...
生成的 HTML 文档加载 JavaScript 胶水文件，并且将 stdout 输出到 [`<textarea>`](/zh-CN/docs/Web/HTML/Element/textarea)。如果应用程序使用了 OpenGL，HTML 文档还会包含一个用来渲染目标的 [`<canvas>`](/zh-CN/docs/Web/HTML/Element/canvas) 标签。修改 Emscripten 的输出文件并将其转换为需要的 Web App 是很容易的。
...
对于想尝试 WebAssembly 而又不需要学习 C 或 Rust 细节的网络开发者来说，AssemblyScript 将是最好的选择。AssemblyScript 将 TypeScript 的严格变体编译为 WebAssembly，让网络开发者可以继续使用他们熟悉的 TypeScript 兼容工具，如 Prettier、ESLint、VS Code IntelliSense 等。你可以在 [https://www.assemblyscript.org/](https://www.assemblyscript.org/) 上查看其文档。
