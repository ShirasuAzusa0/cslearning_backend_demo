...
+   **依赖** 大多为在开发时不会变动的纯 JavaScript。一些较大的依赖（例如有上百个模块的组件库）处理的代价也很高。依赖也通常会存在多种模块化格式（例如 ESM 或者 CommonJS）。
    ...
    Vite 将会使用 [esbuild](https://esbuild.github.io/) [预构建依赖](https://vitejs.cn/vite3-cn/guide/dep-pre-bundling.html)。esbuild 使用 Go 编写，并且比以 JavaScript 编写的打包器预构建依赖快 10-100 倍。
...
要确保开发服务器和生产环境构建之间的最优输出和行为一致并不容易。所以 Vite 附带了一套 [构建优化](https://vitejs.cn/vite3-cn/guide/features.html#build-optimizations) 的 [构建命令](https://vitejs.cn/vite3-cn/guide/build.html)，开箱即用。
...
你可以查看 [比较](https://vitejs.cn/vite3-cn/guide/comparisons.html) 章节获取更多细节，了解 Vite 与同类工具的异同。
