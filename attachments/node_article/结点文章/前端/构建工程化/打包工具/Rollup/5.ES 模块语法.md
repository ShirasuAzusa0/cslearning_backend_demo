...
### 名称空间导入[](#namespace-imports "名称空间导入的直接链接")
...
### 无命名导入[](#empty-import "无命名导入的直接链接")

加载模块代码，但不要使任何新对象可用。

这对于 polyfill 很有用，或者当导入代码的主要目的是处理 prototypes 时。

### 动态导入[](#dynamic-import "动态导入的直接链接")

使用 [动态导入 API](https://github.com/tc39/proposal-dynamic-import#import) 导入模块。

```js
import('./modules.js').then(({ default: DefaultExport, NamedExport }) => {	// 用这个模块做点什么...});
```

这对于代码分解应用程序和动态使用模块非常有用。

## 导出[](#exporting "导出的直接链接")

### 具名导出[](#named-exports "具名导出的直接链接")
...
```js
// `var`, `let`, `const`, `class`, 和 `function` 都是有效的export const something = true;
```

### 默认导出[](#default-export "默认导出的直接链接")
...
## 绑定是如何工作的？[](#how-bindings-work "绑定是如何工作的？的直接链接")
...
```js
// incrementer.jsexport let count = 0;export function increment() {	count += 1;}
```

```js
// main.jsimport { count, increment } from './incrementer.js';console.log(count); // 0increment();console.log(count); // 1count += 1; // Error - 只有 incrementer.js 可以更改这个变量
```